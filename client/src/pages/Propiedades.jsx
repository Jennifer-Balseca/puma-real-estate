...
const Propiedades = () => {
  ...
  // Filters
  const [modalidadFilters, setModalidadFilters] = useState([]);
  ...
  const [requireParqueadero, setRequireParqueadero] = useState(null);
  ...
  useEffect(() => {
    ...
    setUbicacionText(qParam);
    setCityFilter(ciudadParam);
    setModalidadFilters(modalidadParam ? modalidadParam.split(',').map((s) => s.trim()) : []);
    setTipoFilters(tipoParam ? tipoParam.split(',').map((s) => s.trim()) : []);
    setPriceFrom(pf);
    setPriceTo(pt);
  }, []);
  ...
  const filtered = useMemo(() => {
    return allProperties.filter((p) => {
      ...
      // modalidad 
      const matchesModalidad = modalidadFilters.length === 0 || (p.modalidad && modalidadFilters.map((m) => m.toLowerCase()).includes((p.modalidad || '').toLowerCase()));

      // tipo
      const matchesTipo = tipoFilters.length === 0 || (p.tipo && tipoFilters.map((t) => t.toLowerCase() === 'lote' ? 'terreno' : t.toLowerCase()).includes((p.tipo || '').toLowerCase()));

      // precio
      const minP = priceFrom === '' ? 0 : Number(priceFrom);
      const maxP = priceTo === '' ? Infinity : Number(priceTo);
      const matchesPrice = price >= (Number.isFinite(minP) ? minP : 0) && price <= (Number.isFinite(maxP) ? maxP : Infinity);

      // ubicacion 
      const locationText = (ubicacionText || '').toLowerCase().trim();
      ...
      // ciudad
      const cityText = (cityFilter || '').toLowerCase();
      ...
      const matchesRooms = minRooms === '' || (!Number.isNaN(roomsVal) && roomsVal >= Number(minRooms));
      const matchesBanos = minBanos === '' || (!Number.isNaN(bathsVal) && bathsVal >= Number(minBanos));

      // parqueadero
      const matchesParqueadero = requireParqueadero === null || (requireParqueadero === true ? Boolean(p.caracteristicas?.parqueadero) : true);
  ...
  }, [allProperties, modalidadFilters, tipoFilters, priceFrom, priceTo, ubicacionText, cityFilter, minRooms, minBanos, requireParqueadero, q]);
      ...
      <section className="mb-8">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-6">
          <div>
            ...
            <p className="text-neutral-400 max-w-xl">Curaduría de las propiedades arquitectónicas más prestigiosas del mercado actual.</p>
          </div>
        ...
        <div className="bg-surface-container-low p-6 border border-surface-variant flex flex-wrap gap-6">
          <div className="flex-1 min-w-[200px]">
            ...
            <input value={ubicacionText} onChange={(e) => setUbicacionText(e.target.value)} placeholder="Ej. paseo san francisco" className="w-full bg-surface-container border border-surface-variant text-white p-3 outline-none" />
          </div>
          ...
          <div className="flex-1 min-w-[200px] relative" ref={modalRef}>
            ...
            <button type="button" onClick={() => setShowModalidadDropdown((s) => !s)} className="w-full text-left bg-surface-container border border-surface-variant text-white p-3 rounded flex items-center justify-between">
              <div className="truncate">
                {modalidadFilters.length === 0 ? <span className="text-neutral-400">Seleccione transacción</span> : <span className="text-white">{modalidadFilters.join(', ')}</span>}
              </div>
              ...
              <div className="absolute left-0 mt-2 w-full bg-[#0F0F0F] border border-surface-variant rounded shadow-lg z-40 p-3">
                {['Venta', 'Alquiler'].map((m) => (
                  <label key={m} className="flex items-center gap-2 text-neutral-300 mb-2">
                    <input type="checkbox" checked={modalidadFilters.includes(m)} onChange={(e) => setModalidadFilters((s) => e.target.checked ? [...s, m] : s.filter((x) => x !== m))} />
                    <span>{m}</span>
                  </label>
                ))}
              </div>
          ...
          <div className="flex-1 min-w-[220px] relative" ref={tipoRef}>
            ...
            <button type="button" onClick={() => setShowTipoDropdown((s) => !s)} className="w-full text-left bg-surface-container border border-surface-variant text-white p-3 rounded flex items-center justify-between">
              <div className="truncate">
                {tipoFilters.length === 0 ? <span className="text-neutral-400">Seleccione tipo</span> : <span className="text-white">{tipoFilters.join(', ')}</span>}
              </div>
              <span className="material-symbols-outlined text-neutral-400">expand_more</span>
            </button>
              ...
              <div className="absolute left-0 mt-2 w-full bg-[#0F0F0F] border border-surface-variant rounded shadow-lg z-40 p-3">
                {types.map((t) => (
                  <label key={t} className="flex items-center gap-2 text-neutral-300 mb-2">
                    <input type="checkbox" checked={tipoFilters.includes(t)} onChange={(e) => setTipoFilters((s) => e.target.checked ? [...s, t] : s.filter((x) => x !== t))} />
                    <span>{t}</span>
                  </label>
                ))}
              </div>
          ...
          <div className="flex-1 min-w-[160px]">
            ...
            <input value={cityFilter} onChange={(e) => setCityFilter(e.target.value)} placeholder="Ej. Quito" className="w-full bg-surface-container border border-surface-variant text-white p-3 outline-none" />
          </div>

          <div className="flex-1 min-w-[240px]">
            <label className="block text-sm text-neutral-400 mb-2">Rango de Precio</label>
            <div className="flex gap-2">
              <input type="number" placeholder="200" value={priceFrom} onChange={(e) => setPriceFrom(e.target.value)} className="w-1/2 bg-transparent border border-surface-variant p-2 text-white" />
              <input type="number" placeholder="500" value={priceTo} onChange={(e) => setPriceTo(e.target.value)} className="w-1/2 bg-transparent border border-surface-variant p-2 text-white" />
            </div>
          ...
          <div className="flex items-center">
            <button onClick={() => { setTipoFilters([]); setModalidadFilters([]); setUbicacionText(''); setCityFilter(''); setPriceFrom(''); setPriceTo(''); setMinRooms(''); setMinBanos(''); setRequireParqueadero(null); }} className="h-12 px-6 bg-surface-variant text-white border border-surface-variant">Limpiar</button>
          </div>
          ...
          <div className="w-full">
            ...
            {showMoreFilters && (
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                <input type="number" placeholder="Habitaciones mínimas" value={minRooms} onChange={(e) => setMinRooms(e.target.value)} className="bg-transparent border border-surface-variant p-2 text-white" />
                <input type="number" placeholder="Baños mínimos" value={minBanos} onChange={(e) => setMinBanos(e.target.value)} className="bg-transparent border border-surface-variant p-2 text-white" />
                <div className="flex items-center gap-2 relative" ref={parqueRef}>
                  ...
                  <button type="button" onClick={() => setShowParqueDropdown((s) => !s)} className="bg-transparent border border-surface-variant p-2 text-white flex items-center gap-2">
                    <span>{requireParqueadero === null ? 'Cualquiera' : requireParqueadero ? 'Con parqueadero' : 'Sin parqueadero'}</span>
                    <span className="material-symbols-outlined text-neutral-400">expand_more</span>
                  </button>

                  {showParqueDropdown && (
                    <div className="absolute right-0 mt-10 w-48 bg-[#0F0F0F] border border-surface-variant rounded shadow-lg z-40 p-3">
                      <button onClick={() => { setRequireParqueadero(null); setShowParqueDropdown(false); }} className="w-full text-left py-2 text-neutral-300">Cualquiera</button>
                      <button onClick={() => { setRequireParqueadero(true); setShowParqueDropdown(false); }} className="w-full text-left py-2 text-neutral-300">Con parqueadero</button>
                      <button onClick={() => { setRequireParqueadero(false); setShowParqueDropdown(false); }} className="w-full text-left py-2 text-neutral-300">Sin parqueadero</button>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filtered.length === 0 ? (
            ...
            filtered.map((property) => (
              <article key={property._id} className="group cursor-pointer" onClick={() => handleOpenProperty(property)}>
                <div className="relative aspect-[4/5] overflow-hidden mb-4 bg-surface-container">
                  <img loading="lazy" src={(property.imagenes && property.imagenes[0]) || property.mediaUrls?.[0] || ''} alt={property.titulo} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  
                </div>

                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-h1 text-xl text-white">{property.titulo}</h3>
                  <span className="font-subtitle text-primary-container">${Number(property.precio).toLocaleString()}</span>
                </div>

                <p className="text-sm text-neutral-400 mb-3 flex items-center gap-2"><span className="material-symbols-outlined">location_on</span>{property.ubicacion?.direccion || 'Sin ubicación'}</p>
                ...
                {(property.amenidades?.length > 0 || property.caracteristicas?.amenidades?.length > 0) && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {[...(property.amenidades || []), ...(property.caracteristicas?.amenidades || [])]
                      .slice(0, 3)
                      .map((amenidad, idx) => (
                        <span key={idx} className="text-[10px] uppercase tracking-wider bg-surface-variant text-neutral-300 px-2 py-1 rounded">
                          {amenidad}
                        </span>
                      ))
                    }
                    {(property.amenidades?.length + property.caracteristicas?.amenidades?.length > 3) && (
                      <span className="text-[10px] text-primary-container px-1 py-1">...</span>
                    )}
                  </div>
                )}
              </article>
