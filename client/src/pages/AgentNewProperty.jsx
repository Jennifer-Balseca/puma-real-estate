...
const AgentNewProperty = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setSuccessMessage(response.data?.message || (isEditing ? 'Cambios guardados correctamente.' : 'Propiedad registrada correctamente.'));
      emitPropertiesRefresh();
        ...
        <div className="mb-8 space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="inline-flex items-center gap-2 border border-white/10 bg-white/5 backdrop-blur-sm px-4 py-3 md:py-2 text-xs uppercase tracking-widest text-on-surface transition-colors hover:border-[#D4AF37] hover:text-[#D4AF37]"
            >
        ...
        <form onSubmit={handleSubmit} className="grid gap-5 border border-neutral-800 bg-black/80 p-6 md:grid-cols-2 md:p-8">
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <textarea
              ...
              onChange={handleChange}
              className="w-full border border-white/10 bg-white/5 px-4 py-3 text-white outline-none focus:border-[#D4AF37] focus:shadow-[0_0_10px_rgba(212,175,55,0.2)] transition-all"
          ...
          <div className="md:col-span-2 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <label className="flex items-end gap-3 border border-white/10 bg-white/5 px-4 py-3 text-sm text-white">
              <input
                id="parqueadero"
                name="parqueadero"
                type="checkbox"
          ...
          <div className="md:col-span-2 flex flex-col gap-3 sm:flex-row">
            <button
              type="submit"
              disabled={loading}
              className="py-4 md:py-3 flex-1 bg-[#D4AF37] px-6 text-sm font-semibold uppercase tracking-[0.2em] text-black transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-70"
            >
              {loading ? 'Guardando...' : isEditing ? 'Guardar cambios' : 'Registrar propiedad'}
            </button>

            <button
              type="button"
              onClick={() => navigate(role === 'Admin' ? '/admin/propiedades' : '/agente/inventario')}
              className="py-4 md:py-3 border border-neutral-700 px-6 text-sm uppercase tracking-[0.2em] text-[#C0C0C0] transition hover:border-[#D4AF37] hover:text-[#D4AF37]"
            >
              Ver inventario
            </button>
          </div>
